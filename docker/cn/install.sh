...
# 使用清华源安装 pytorch
pip install /wheels/*.whl torch${TORCH_VERSION} -i https://pypi.tuna.tsinghua.edu.cn/simple --extra-index-url https://download.pytorch.org/whl/${CUDA_VERSION_SHORT}


if [[ "${CUDA_VERSION_SHORT}" != "cu118" ]] && [[ "${PYTHON_VERSION}" != "3.9" ]]; then
    pip install cuda-python dlblas -i https://pypi.tuna.tsinghua.edu.cn/simple
fi
